...
function handleProfile(event) {
  ...
  const target = Math.round(Math.max(1200, bmr * Number(get('activity')) * Number(get('goal'))) / 50) * 50;
  const targetWeight = editing ? (state.profile.targetWeight || 75) : Number(get('targetWeight'));
  state.profile = { name: userName(), gender: get('gender'), activity: Number(get('activity')), goal: Number(get('goal')), age, height, weight, targetWeight, registeredAt: state.profile?.registeredAt || dayKey(), target, protein: Math.round(weight * 1.6), fats: Math.round(weight * .8), carbs: Math.round((target - (weight * 1.6 * 4) - (weight * .8 * 9)) / 4) };
  state.planDate = dayKey();
  state.foodLog = {};
...
function refreshDashboardData() {
  ...
  const weightProgress = data.startWeight > targetWeight ? Math.max(0, Math.min(100, Math.round((data.startWeight - currentWeight) / (data.startWeight - targetWeight) * 100))) : 0;
